...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function go(id) {
    SCREENS.forEach(s => {
      const screen = document.getElementById('sc-'+s);
    ...
    tabs.forEach((t,i) => t.classList.toggle('on', SCREENS[i]===id));
    tabs.forEach((t,i) => {
      t.onclick = () => go(SCREENS[i]);
    });
    // Si entramos a la ruleta específicamente, asegurar que se dibuje
    if(id === 'roulette') initRouleta();
  }
  ...
  const pdots = document.getElementById('pdots');
  if(pdots){for(let i=0;i<20;i++){const d=document.createElement('div');d.className='pdot';pdots.appendChild(d);}}
  function updDots(){if(pdots) pdots.querySelectorAll('.pdot').forEach((d,i)=>d.classList.toggle('on',i<sLines));}
  updDots();

  function buildReels(){
    const rr=document.getElementById('reelrow');
    if(!rr) return;
    rr.innerHTML='';sCells=[];
    for(let c=0;c<3;c++){
  ...
  function updSlotsUI(){
    const sc = document.getElementById('slotsCredits');
    if(sc) sc.textContent=fmt(sCredits);
    document.getElementById('gcr').textContent=fmt(sCredits);
    GC=sCredits;
    const sbv = document.getElementById('sbv');
    if(sbv) sbv.textContent=sBet;
    const tb = document.getElementById('totalBet');
    if(tb) tb.textContent=fmt(sBet*sLines);
  }
  ...
  async function spin(){
    ...
    if(hl) hl.innerHTML=sHist.map(h=>`<div class="hi"><span class="hsyms">${h.r.join(' ')}</span><span class="hbet">-$${fmt(h.b)}</span><span class="hres ${h.w>0?'w':'l'}">${h.w>0?'+$'+fmt(h.w):'-$'+fmt(h.b)}</span></div>`).join('');
  }
  ...
  function initRouleta() {
    roulCanvas = document.getElementById('roulCanvas3D');
    if (!roulCanvas) return;
    rctx = roulCanvas.getContext('2d');
    
    // Grid de números
    const ng=document.getElementById('numgrid');
    if(ng && ng.children.length === 0){
      for(let i=0;i<=36;i++){
  ...
  function updRoulSel(){
    ...
    const selList = document.getElementById('roulSelList');
    if(!selList) return;
    if(!keys.length){selList.textContent='Sin apuestas seleccionadas';return;}
    ...
    selList.innerHTML='Apostando a: <strong style="color:var(--gold)">'+parts.join(', ')+'</strong> · Costo total: <strong style="color:var(--gold)">$'+fmt(keys.length*roulBetAmt)+'</strong>';
  }
  ...
  function drawWheel(ang){
    if(!rctx)return;
    const W=320, H=320, cx=W/2, cy=W/2;
    const tiltY = 0.45; const rx = W/2 - 6; const ry = rx * (1 - tiltY); 
    rctx.clearRect(0,0,W,H);
    const shadowGrad = rctx.createRadialGradient(cx, cy+10, rx*0.2, cx, cy+20, rx*1.1);
    shadowGrad.addColorStop(0,'rgba(0,0,0,0.55)'); shadowGrad.addColorStop(1,'rgba(0,0,0,0)');
    rctx.save(); rctx.scale(1, 0.25); rctx.fillStyle = shadowGrad; rctx.beginPath();
    rctx.ellipse(cx, (cy+20)/0.25, rx*1.15, rx*1.15, 0, 0, Math.PI*2); rctx.fill(); rctx.restore();
    const depth = 22; const n=RSEQ.length, arc=2*Math.PI/n;
    for(let i=0;i<n;i++){
      const a0 = ang + i*arc; const a1 = a0 + arc;
      const midA = a0 + arc/2; const sinMid = Math.sin(midA);
      if(sinMid > 0) { 
        const num = RSEQ[i]; const col = num===0?'#1a8a40':RED_N.includes(num)?'#8b1111':'#0d0d1e';
        rctx.beginPath(); const x0 = cx + rx * Math.cos(a0); const y0 = cy + ry * Math.sin(a0);
        const x1 = cx + rx * Math.cos(a1); const y1 = cy + ry * Math.sin(a1);
        rctx.moveTo(x0, y0); rctx.lineTo(x1, y1); rctx.lineTo(x1, y1+depth); rctx.lineTo(x0, y0+depth);
        rctx.closePath(); rctx.fillStyle = col; rctx.globalAlpha = 0.7 * sinMid; rctx.fill(); rctx.globalAlpha = 1;
      }
    }
    rctx.save(); rctx.globalAlpha=0.5;
    const rimGrad=rctx.createLinearGradient(cx-rx,cy,cx+rx,cy);
    rimGrad.addColorStop(0,'#3a1b00'); rimGrad.addColorStop(0.5,'#c99522'); rimGrad.addColorStop(1,'#3a1b00');
    rctx.strokeStyle=rimGrad; rctx.lineWidth=depth; rctx.beginPath();
    rctx.ellipse(cx, cy+depth/2, rx+depth/2, ry+depth*0.15, 0, 0, Math.PI); rctx.stroke(); rctx.restore();
    rctx.save(); rctx.beginPath(); rctx.ellipse(cx, cy, rx, ry, 0, 0, Math.PI*2); rctx.clip();
    RSEQ.forEach((num,i)=>{
      const a=ang+i*arc; rctx.beginPath(); rctx.moveTo(cx,cy); const steps=8;
      for(let s=0;s<=steps;s++){
        const aa = a + arc*(s/steps); const ex = cx + rx * Math.cos(aa); const ey = cy + ry * Math.sin(aa);
        if(s===0) rctx.lineTo(ex,ey); else rctx.lineTo(ex,ey);
      }
      rctx.closePath(); const col=num===0?'#22aa55':RED_N.includes(num)?'#cc2222':'#1a1a2e';
      const midA=ang+i*arc+arc/2; const lightFactor = 0.7 + 0.4*(Math.cos(midA)*-0.5+0.5);
      rctx.fillStyle=col; rctx.globalAlpha=lightFactor; rctx.fill(); rctx.globalAlpha=1;
      rctx.strokeStyle='rgba(226,184,75,0.3)'; rctx.lineWidth=0.5; rctx.stroke();
      rctx.save(); const tx=cx+rx*0.77*Math.cos(a+arc/2); const ty=cy+ry*0.77*Math.sin(a+arc/2);
      rctx.translate(tx,ty); rctx.transform(1,0,0,(ry/rx), 0, 0); rctx.rotate(a+arc/2+Math.PI/2);
      rctx.fillStyle='#f0e6cc'; rctx.globalAlpha = 0.5+0.5*(Math.cos(midA)*-0.5+0.5);
      rctx.font='bold 8px Rajdhani,sans-serif'; rctx.textAlign='center'; rctx.textBaseline='middle';
      rctx.fillText(num,0,0); rctx.restore();
    });
    rctx.restore();
    const rimG=rctx.createRadialGradient(cx-rx*0.3,cy-ry*0.3,rx*0.8,cx,cy,rx+5);
    rimG.addColorStop(0,'rgba(255,215,100,0.0)'); rimG.addColorStop(0.8,'rgba(200,148,30,0.6)'); rimG.addColorStop(1,'rgba(80,40,0,0.9)');
    rctx.beginPath(); rctx.ellipse(cx, cy, rx+4, ry+4, 0, 0, Math.PI*2); rctx.strokeStyle=rimG; rctx.lineWidth=8; rctx.stroke();
    const capRx=20, capRy=capRx*(ry/rx);
    const capG=rctx.createRadialGradient(cx-6,cy-capRy*0.4,2,cx,cy,capRx);
    capG.addColorStop(0,'#fff8e0'); capG.addColorStop(0.5,'#d2a53d'); capG.addColorStop(1,'#6a4300');
    rctx.beginPath(); rctx.ellipse(cx, cy, capRx, capRy, 0, 0, Math.PI*2); rctx.fillStyle=capG; rctx.fill();
    rctx.strokeStyle='rgba(226,184,75,0.8)'; rctx.lineWidth=1.5; rctx.stroke();
    if(roulSpinning){
      const ballAng = -ang * 1.5; const ballRx = rx * 0.88; const ballRy = ballRx * (ry/rx);
      const bx = cx + ballRx * Math.cos(ballAng); const by = cy + ballRy * Math.sin(ballAng);
      const ballG=rctx.createRadialGradient(bx-2,by-2,1,bx,by,5);
      ballG.addColorStop(0,'#ffffff'); ballG.addColorStop(1,'#cccccc');
      rctx.beginPath(); rctx.arc(bx,by,5,0,Math.PI*2); rctx.fillStyle=ballG; rctx.fill();
      rctx.strokeStyle='rgba(150,150,150,0.5)'; rctx.lineWidth=0.5; rctx.stroke();
    }
  }

  let roulIdleAnim=null;
  function startIdleRoul(){
    if(roulSpinning)return;
    let t=0;
    function idleFrame(){
      if(roulSpinning){roulIdleAnim=null;return;}
      t+=0.003;
      drawWheel(t);
      roulIdleAnim=requestAnimationFrame(idleFrame);
    }
    roulIdleAnim=requestAnimationFrame(idleFrame);
  }
  ...
  async function roulSpin(){
    ...
    roulSpinning=true;
    if(roulIdleAnim){cancelAnimationFrame(roulIdleAnim);roulIdleAnim=null;}
    ...
    const stage=document.getElementById('roul3DStage');
    if(stage){stage.style.transform='rotateX(58deg)';}
  ...
  function pkDraw(){
    ...
    if(res.m>0){
      ...
      if(win>bestWin){bestWin=win;document.getElementById('sBest').textContent='$'+fmt(win);}
      toast('✨ '+res.n+' → +$'+fmt(win));
  ...
  if(document.getElementById('players')){renderPlayers();setInterval(renderPlayers,7000);}
  ...
  const MENU=[
    ...
    {cat:'vino',img:'../img/vino.jpeg',name:'Botella Champagne Brut',desc:'Champagne Brut Imperial. Burbujas finas. Ideal para celebrar tus victorias de la noche',price:'$18.000',badge:'vip'},
  ...
  function renderGastro(){
    ...
    const catClass={cocktail:'gc-bebida',vino:'gc-bebida',espirituoso:'gc-bebida',sin_alcohol:'gc-bebida',caliente:'gc-bebida'};
    grid.innerHTML=items.map(it=>`
      <div class="gastro-card ${catClass[it.cat]||''}" onclick="addOrder('${it.name.replace(/'/g,'')}')">
        <div class="gastro-card-banner"><img src="${it.img}" alt="${it.name}" loading="lazy"></div>
  ...
  function validatePMForm(){
    if(!selectedPM){
      document.querySelectorAll('.dep-payment-method').forEach(el=>{
  ...
  function updateDepBtn(){
    ...
    if(!btn) return;
    const notice=document.getElementById('dep-pm-required');
    const ready = depAmount>=3000 && selectedPM!==null;
    btn.disabled=!ready;
    if(depAmount>0&&depAmount<3000){
      btn.textContent='Mínimo $3.000';
